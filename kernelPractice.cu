	...
	long** left, long** right, long** result, bool useSharedMem)
{
	...
	unsigned int blocksPerRow = rightCols / TILE_WIDTH;
	// Bring to next multiple of TILE_WIDTH so there are suffiecient blocks per row for rectangles
	if(rightCols % TILE_WIDTH != 0)
	{
		blocksPerRow++;;
	}

	__shared__ long sharedData[TILE_WIDTH * 2][SHARED_PER_ROW]; // First TILE_WIDTH for rows of left, rest for cols of right
	unsigned int sharedMemOffset = 0; // Keeps track of what section of the rows are in shared memory currently

	// Grid stride
	while(true)
	{
		unsigned int firstRow = (blockId / blocksPerRow) * TILE_WIDTH; // Of the tile
		unsigned int firstCol = (blockId % blocksPerRow) * TILE_WIDTH; // Of the tile
		unsigned int rowOffset = threadIdx.x / TILE_WIDTH; // Which row (with respect to start) this thread occupies
		unsigned int colOffset = threadIdx.x % TILE_WIDTH; // Which column (with respect to start) this thread occupies

		// Position in the output matrix's tile (one thread per output element)
		...
		// Keep going through the rows until everything has gone through shared memory
		do
		{
			if(useSharedMem)
			{
				// Col offset determines portion of its row it handles
				unsigned int rowMemStart = ((SHARED_PER_ROW / TILE_WIDTH) * colOffset) + sharedMemOffset;

				// Row offset determines portion of its col it handeles
				unsigned int colMemStart = ((SHARED_PER_ROW / TILE_WIDTH) * rowOffset) + sharedMemOffset;

				// Load left matrix rows into shared memory using
				for(unsigned int i = 0; i < (SHARED_PER_ROW / TILE_WIDTH) && rowMemStart + i < shared; i++)
				{
					sharedData[rowOffset][i] = left[firstRow + rowOffset][rowMemStart + i];
				}

				// Load right matrix columns in as rows (uses transposed coordinates)
				for(unsigned int i = 0; i < (SHARED_PER_ROW / TILE_WIDTH) && colMemStart + i < shared; i++)
				{
					sharedData[TILE_WIDTH + colOffset][i] = right[colMemStart + i][firstCol + colOffset];
				}

				__syncthreads(); // Ensure all memory is loaded
			}
			...
			if(useSharedMem)
			{
				// Do the dot product to get the final value
				for(unsigned int i = 0; i < SHARED_PER_ROW && i < shared; i++)
				{
					sum += sharedData[rowOffset][sharedMemOffset + i] * sharedData[TILE_WIDTH + colOffset][sharedMemOffset + i];
				}
			}
			// Grab data directly from the matrices
			else
			{
				// Do the dot product to get the final value
				for(unsigned int i = sharedMemOffset; i < shared; i++)
				{
			...
			if(useSharedMem)
			{
				__syncthreads(); // All threads must finish computations before overriding shared memory
			}

			result[row][col] += sum;

			// Update offset to next iteration uses new elements
			if(useSharedMem)
			{
				sharedMemOffset += SHARED_PER_ROW;
			}
		} while(sharedMemOffset < shared && useSharedMem); // Multiple passes only needed with shared memory

		blockId += gridDim.x;
		sharedMemOffset = 0; // Reset offset for grid-striding
	}
}
